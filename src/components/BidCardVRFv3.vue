<template>
  <div class="bid-card rounded-xl flex flex-col overflow-hidden" style="width: 100%;box-shadow: 0px 20px 40px 0px #0000001a" :class="darkMode ? 'dark-mode-surface' : 'light-mode-background'">
    <div class="top-part flex flex-col p-8">
      ...
      <button class="button primary mt-6" v-if="!requireAdminCommitVRF && !isCollectableActive && isVRFSale && hasRequestedVRF && hasFulfilledVRF && !hasCommittedVRF && !isSubmittingRandomnessCommitment" @click="startRandomnessCommitment">
        Commit Chainlink Randomness
      </button>
      <button class="button primary disabled mt-6" v-if="requireAdminCommitVRF && !isCollectableActive && isVRFSale && hasRequestedVRF && hasFulfilledVRF && !hasCommittedVRF && !isSubmittingRandomnessCommitment">
        Awaiting Reveal
      </button>
...
console.log({'props.price': props.price})
    ...
    watchEffect(() => {
      if(props.collectable) {
        collectableData.value = props.collectable;
      }
    })
    ...
    watchEffect(async () => {
      if (account?.value && isAuction?.value && !props.isOpenEdition && !props?.isCollectableActive && collectableData?.value?.contract_address && collectableData.value.nft_token_id) {
        ...
        if(collectableData?.value?.vrf_version !== 2 && collectableData?.value?.vrf_version !== 3) {
    ...
    const fieldValidatorSale = (value) => {
      if (value) {
        if(value <= 50) {
          ...
          return 'Maximum 50 per transaction'
        }
      } else {
        return true;
      }
    ...
    watchEffect(() => {
      ...
      console.log({'collectableData.value BidCard': collectableData.value})
    })
    ...
    watchEffect(() => {
      if(props.customPaymentTokenAddress) {
    ...
    const checkCustomPaymentTokenAllowance = async (saleContractAddress, requiredAllowance) => {
      console.log({'saleContractAddress': saleContractAddress, 'customPaymentToken.value': customPaymentToken.value})
      if(saleContractAddress && customPaymentToken.value) {
        console.log({requiredAllowance})
        const currentAllowance = await customPaymentToken.value.allowance(account.value, saleContractAddress);
        console.log({'currentAllowance.toString()': currentAllowance.toString()})
    ...
    watchEffect(() => {
      if(customPaymentTokenData.customPaymentTokenDecimals && (auctionField.value || saleField.value)) {
        ...
        console.log({requiredAllowance, 'price.value': price.value})
